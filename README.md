...
- **Start deleting a reference and the plugin offers to finish it.** Delete the space after a linked reference and a suggestion appears, offering to *delete* it, to *edit* it (the link goes, the text stays and the caret lands at its end, so a wrong verse is corrected by typing over it), or to cancel. The counterpart to the suggestion that appears once a reference has been typed — same idea at the other end of its life. Scripture links only: songs and source citations are written as jw.org links, so they can never trigger it. This is the route that works on a phone, where the link is rendered rather than shown as source.
