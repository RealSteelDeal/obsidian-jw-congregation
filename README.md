...
The day's overview note is merged the same way (one marker per session, so its programme list stays intact). A field you corrected through the plugin's own remove/adjust offer is marked as yours (an invisible flag on the marker that block already carries) and left alone by every later update — otherwise correcting a song the congregation actually sang would have reverted on the next run. Its neighbours keep updating as before.
...
src/
  ...
  util/
    ...
    parseErrors.ts           # structured parse errors, translated per UI language instead of hardcoded
    decompressionGuard.ts    # size limits against zip-bomb-like decompression
